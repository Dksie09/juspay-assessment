...
function SidebarGroupLabel({ className, asChild = false, ...props }) {
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-foreground-muted ring-sidebar-ring flex h-8 py-1 shrink-0 items-center rounded-md px-2 text-sm font-normal leading-5 [font-feature-settings:'ss01','cv01','cv11'] outline-hidden transition-[margin,opacity] duration-200 ease-linear [&>svg]:size-4 [&>svg]:shrink-0",
...
function SidebarMenu({ className, ...props }) {
  ...
  const handleKeyDown = React.useCallback((e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      const prevIndex =
        currentIndex === -1 ? items.length - 1 : Math.max(0, currentIndex - 1);
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex  items-center gap-1 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
  ...
  const button = (
    <Comp
      ...
      className={cn(
        ...
        // Only apply flex classes when NOT using asChild
        !asChild && "relative flex items-center gap-1 px-3 py-2",
        isActive && "bg-foreground-muted pl-5", // active bg
        className
      )}
      {...props}
    >
